...
namespace rworld
{
    ...
    public:
        ...
        float get_solar_angle(float longitude, float latitude, float current_time) const
        {
            ...
            // Wrap to 0-24 range
            while (local_solar_time < 0.0f)
                local_solar_time += 24.0f;
            while (local_solar_time >= 24.0f)
                local_solar_time -= 24.0f;
        ...
        float get_wind_direction(float longitude, float latitude, float altitude) const
        {
            ...
            // Normalize to 0-360
            while (base_direction < 0.0f)
                base_direction += 360.0f;
            while (base_direction >= 360.0f)
                base_direction -= 360.0f;
        ...
        float get_current_wind_direction(float longitude, float latitude, float altitude, float current_time) const
        {
            ...
            // Normalize to 0-360
            while (current_dir < 0.0f)
                current_dir += 360.0f;
            while (current_dir >= 360.0f)
                current_dir -= 360.0f;
