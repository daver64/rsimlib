...
namespace recs
{
    ...
    public:
        ...
        World(World &&other) noexcept
            : generations_(std::move(other.generations_)), free_ids_(std::move(other.free_ids_)), archetypes_(std::move(other.archetypes_)), entity_locations_(std::move(other.entity_locations_)), resources_(std::move(other.resources_)), event_handlers_(std::move(other.event_handlers_))
        {
        }
...
#pragma omp parallel for schedule(dynamic)
        ...
        void print_memory_usage() const
        {
            ...
            std::cout << "Entity metadata: " << ((generations_.size() * sizeof(uint32_t) + entity_locations_.size() * sizeof(Location)) / 1024.0) << " KB\n";
        }
    ...
    private:
        ...
        C *get_unsafe(Entity e)
        {
            ...
            if (!loc.arch)
                return nullptr;
            if (!loc.arch->key.has(component_id<C>()))
                return nullptr;
