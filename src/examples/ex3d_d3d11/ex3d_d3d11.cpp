...
{
    if (!sl::set_graphics_backend(sl::GraphicsBackend::d3d11) ||
        !sl::set_gfx_mode(sl::GFX_AUTODETECT_WINDOWED, 800, 600))
    {
        std::fprintf(stderr, "Failed to initialize D3D11 graphics backend.\n");
        return -1;
    }
    ...
    while (running)
    {
        ...
        sl::gprintf_center(32, {232, 236, 244}, "Direct3D 11 3D example - Escape to exit");
...
int main(int argc, char *argv[])
{
    std::fprintf(stderr, "ex3d_d3d11 is only supported on Windows.\n");
    return 0;
}
#endif
