...
namespace
{
    ...
    bool add_path(const std::filesystem::path &input, const std::filesystem::path &base, std::vector<ZipEntry> &entries, PackStats &stats)
    {
        ...
        if (std::filesystem::is_directory(input, error))
        {
            ...
            {
                if (error || !add_path(child.path(), base, entries, stats))
                    return false;
            }
            return true;
        }
        ...
        if (error || !read_file(input, entry.data))
            return false;
        std::cout << "file: " << entry.name << '\n';
        ++stats.files;
        entry.crc = crc32(0, entry.data.data(), static_cast<uInt>(entry.data.size()));
        if (deflate_data(entry.data, entry.compressed) && entry.compressed.size() < entry.data.size())
            entry.method = 8;
        else
            entry.compressed = entry.data;
    ...
    bool write_archive(const std::filesystem::path &outputPath, std::vector<ZipEntry> &entries)
    {
        ...
        {
            if (entry.name.size() > UINT16_MAX || entry.data.size() > UINT32_MAX || entry.compressed.size() > UINT32_MAX || archive.size() > UINT32_MAX)
                return false;
