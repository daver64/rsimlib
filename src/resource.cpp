...
std::vector<std::uint8_t> read_file(const std::string& path) {
    ...
    if (size < 0 || static_cast<std::uintmax_t>(size) > std::numeric_limits<std::size_t>::max()) return {};
    std::vector<std::uint8_t> bytes(static_cast<std::size_t>(size));
    file.seekg(0);
    if (!bytes.empty() && !file.read(reinterpret_cast<char*>(bytes.data()), size)) return {};
    return bytes;
}
...
bool Archive::open(const std::string& path) {
    ...
    for (;;) {
        if (read32(bytes, end) == endSignature) break;
        if (end == searchStart) {
...
bool Archive::contains(const std::string& name) const {
    return std::any_of(entries_.begin(), entries_.end(), [&name](const Entry& entry) { return entry.name == name; });
}
...
std::vector<std::uint8_t> Archive::read(const std::string& name) const {
    const auto found = std::find_if(entries_.begin(), entries_.end(), [&name](const Entry& entry) { return entry.name == name; });
