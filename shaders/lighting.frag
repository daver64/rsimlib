...
layout(std430, binding = 3) readonly buffer TileCounts { uint tileCounts[]; };
layout(std430, binding = 4) readonly buffer TileIndices { uint tileIndices[]; };
in vec2 uv;
out vec4 fragColor;
void main() {
    vec2 lightUv = uv;
    if (flipVertical != 0) lightUv.y = 1.0 - lightUv.y;
