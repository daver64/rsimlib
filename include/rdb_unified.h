...
namespace rdb
{
    ...
    public:
        ...
        {
            std::vector<T> rows;
            forEachRow([&](IStatement &statement)
                       { rows.push_back(mapper(statement)); });
            return rows;
        }
    };
    ...
    private:
        ...
        {
            if (result != SQLITE_OK)
                throw UnifiedException("SQLite bind failed");
        }
    };
    ...
    public:
        ...
        void bindNull(int index) override
        {
            ...
            values_[index - 1].clear();
            parameters_[index - 1] = nullptr;
        }
        bool step() override
        {
            if (!result_)
        ...
        std::string getText(int column) const override
        {
            if (!result_ || PQgetisnull(result_, row_, column))
                return "";
    ...
    private:
        static std::string translatePlaceholders(const std::string &sql)
        {
            ...
            for (size_t i = 0; i < sql.size(); ++i)
            {
                if (sql[i] == '\'' && (i == 0 || sql[i - 1] != '\\'))
                    quoted = !quoted;
                if (sql[i] == '?' && !quoted)
                    translated += "$" + std::to_string(++parameter);
                else
                    translated += sql[i];
            }
            return translated;
        ...
        {
            if (index < 1)
                throw UnifiedException("Parameter indexes start at 1");
            if (static_cast<int>(values_.size()) < index)
                values_.resize(index);
            if (static_cast<int>(parameters_.size()) < index)
                parameters_.resize(index, nullptr);
        }
        void set(int index, const std::string &value)
        {
            ensure(index);
            values_[index - 1] = value;
            parameters_[index - 1] = values_[index - 1].c_str();
        }
        void execute()
        {
    ...
    private:
        void set(int index, const std::string &value, bool is_null = false)
        {
            ...
            if (static_cast<int>(values_.size()) < index)
                values_.resize(index);
            values_[index - 1] = value;
            if (parameters_.size() < values_.size())
                parameters_.resize(values_.size());
            if (nulls_.size() < values_.size())
                nulls_.resize(values_.size(), 0);
        ...
        void execute()
        {
            if (!parameters_.empty() && mysql_stmt_bind_param(statement_, &parameters_[0]) != 0)
                throw UnifiedException(mysql_stmt_error(statement_));
            ...
            buffers_.resize(count, std::vector<char>(4096));
            lengths_.resize(count);
            nulls_.resize(count);
            for (unsigned int i = 0; i < count; ++i)
            {
                std::memset(&results_[i], 0, sizeof(MYSQL_BIND));
                results_[i].buffer_type = MYSQL_TYPE_STRING;
                results_[i].buffer = &buffers_[i][0];
                results_[i].buffer_length = buffers_[i].size();
                results_[i].length = &lengths_[i];
                results_[i].is_null = &nulls_[i];
            }
            if (mysql_stmt_bind_result(statement_, &results_[0]) != 0 || mysql_stmt_store_result(statement_) != 0)
                throw UnifiedException(mysql_stmt_error(statement_));
            executed_ = true;
        }
    };
    ...
    public:
        ...
        void execute(const std::string &sql) override
        {
            if (mysql_query(connection_, sql.c_str()) != 0)
                throw UnifiedException(mysql_error(connection_));
        }
