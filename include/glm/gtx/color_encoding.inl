...
namespace glm
{
	...
	GLM_FUNC_QUALIFIER vec<3, T, Q> convertLinearSRGBToD65XYZ(vec<3, T, Q> const& ColourLinearSRGB)
	{
		vec<3, T, Q> const M(0.490f, 0.17697f, 0.2f);
		vec<3, T, Q> const N(0.31f,  0.8124f, 0.01063f);
		vec<3, T, Q> const O(0.490f, 0.01f, 0.99f);

		return (M * ColourLinearSRGB + N * ColourLinearSRGB + O * ColourLinearSRGB) * static_cast<T>(5.650675255693055f);
	}

	template<typename T, qualifier Q>
	GLM_FUNC_QUALIFIER vec<3, T, Q> convertLinearSRGBToD50XYZ(vec<3, T, Q> const& ColourLinearSRGB)
	{
		...
		return M * ColourLinearSRGB + N * ColourLinearSRGB + O * ColourLinearSRGB;
	}

	template<typename T, qualifier Q>
	GLM_FUNC_QUALIFIER vec<3, T, Q> convertD65XYZToLinearSRGB(vec<3, T, Q> const& ColourD65XYZ)
	{
		...
		return M * ColourD65XYZ + N * ColourD65XYZ + O * ColourD65XYZ;
	}

	template<typename T, qualifier Q>
	GLM_FUNC_QUALIFIER vec<3, T, Q> convertD65XYZToD50XYZ(vec<3, T, Q> const& ColourD65XYZ)
	{
